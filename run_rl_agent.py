...
if len(sys.argv) < 3:
    ...
    print(
        "Evaluate Usage: python run_rl_agent.py <your-eval-outdir> evaluate <your-existing-inputdir>"
    )
    exit(1)
...
os.system(
    "aws s3 cp {}{}/output/output.tar.gz ./{}/".format(
        s3_output_path, job_name, localout
    )
)
...
sns.publish(
    ...
    Message="Ready to analyze. S3 output folder: {}{}/output and local folder: ./{}".format(
        s3_output_path, job_name, localout
    ),
)
